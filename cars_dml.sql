...
select count(*) as cars_quantity from cars;
...
select count(*) as chevrolet_car_count from cars where brand = 'Chevrolet';

select avg(year_of_issue) as average_year_of_manufacture from cars;
...
select price from cars where brand = 'Mercedes-Benz';

select max(price) as most_expensive, min(price) as cheapest from cars;
(select * from cars order by price desc limit 1) union all (select * from cars order by price asc limit 1);
